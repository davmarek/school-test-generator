<div class="py-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm space-y-6">
        ...
        <flux:text class="mb-6">Choose how many questions to include from each group. Mandatory questions are automatically included.</flux:text>
        ...
        <form wire:submit="generate" class="space-y-6">
            ...
            @foreach($limits as $key => $limit)
                <flux:field>
                    ...
                    <flux:input type="number" wire:model="config.{{ $key }}" min="{{ $limit['min'] }}" max="{{ $limit['max'] }}" />
            ...
            @else
                <div class="pt-4 flex justify-end">
                    <flux:button type="submit" variant="primary" icon="document-arrow-down">Generate & Download PDF</flux:button>
                </div>
            @endif
        </form>
    </div>
