<div class="pt-4 pb-6">
    <flux:button icon="arrow-left" variant="ghost" :href="route('tests.index')" class="mb-6" wire:navigate>Back
    </flux:button>
    <div class="flex flex-wrap items-center gap-4 mb-6">
        <div class="flex-1">
            <h1 class="text-2xl font-bold">{{ $test->name }}</h1>
            <p class="text-sm text-zinc-500 whitespace-nowrap">Max Points: {{ $test->max_points }}</p>
        </div>
        <div class="flex items-center gap-2">
            <flux:modal.trigger name="edit-test">
                <flux:button icon="pencil-square">Edit Details</flux:button>
            </flux:modal.trigger>
            <flux:button variant="primary" :href="route('tests.generate', $test)" wire:navigate>Generate PDF
            </flux:button>
        </div>
    </div>

    <!-- Edit Test Modal -->
    <flux:modal name="edit-test" class="md:min-w-md">
    ...
    <!-- Create Group Modal -->
    <flux:modal name="create-group" class="md:min-w-md">
                ...
                <div class="space-y-3" x-data
                    ...
                    @foreach ($questionForm['options'] as $index => $option)
                        <div class="flex gap-2 items-center" wire:key="option-{{ $index }}">
                            ...
                            <flux:input class="flex-1" wire:model="questionForm.options.{{ $index }}.text"
    ...
    <div class="space-y-8">
        <!-- Ungrouped Questions -->
        <div
            class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm space-y-4">
            <div class="flex justify-between items-center">
                <flux:heading size="lg">Ungrouped Questions</flux:heading>
                <flux:button size="sm" icon="plus" wire:click="openQuestionModal(null)">Add Question
                </flux:button>
            </div>
                ...
                <div class="space-y-2">
                    @foreach ($ungroupedQuestions as $question)
                        <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800 rounded-lg"
                            ...
                            <div class="flex-1 grid grid-cols-2 md:grid-cols-4 place-items-start items-center gap-4">
                                <p class="font-medium col-span-2">{{ $question->text }}</p>
                                <div>
                            ...
                            <div class="flex flex-col md:flex-row gap-1 md:gap-2">
                                <flux:button icon="pencil" size="sm" variant="ghost"
                                    wire:click="openQuestionModal(null, {{ $question->id }})" />
                                <flux:button icon="trash" size="sm" variant="ghost" class="text-red-500"
                                    wire:click="deleteQuestion({{ $question->id }})" wire:confirm="Are you sure?" />
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex gap-2">
                        <flux:button size="sm" icon="plus"
                            wire:click="openQuestionModal({{ $group->id }})">Add Question</flux:button>
                        <flux:button size="sm" icon="trash" variant="ghost" class="text-red-500"
                            wire:click="deleteGroup({{ $group->id }})"
                            wire:confirm="Delete group and its questions?" />
                    </div>
                </div>
                    ...
                    <div class="space-y-2">
                        @foreach ($group->questions as $question)
                            <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800 rounded-lg"
                                wire:key="q-{{ $question->id }}">
                                <div
                                    class="flex-1 grid grid-cols-2 md:grid-cols-4 place-items-start items-center gap-4">
                                    <p class="font-medium col-span-2">{{ $question->text }}</p>
                                    <div>
                                        <flux:badge size="sm">{{ $question->type->label() }}</flux:badge>
                                        @if ($question->is_mandatory)
